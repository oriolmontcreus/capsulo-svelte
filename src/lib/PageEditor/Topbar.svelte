...
<header class="border-border flex h-14 shrink-0 items-center justify-between gap-3 border-b px-3">
	<!-- Left cluster -->
	<div class="flex min-w-0 items-center gap-2">
		<DropdownMenu.Root>
			<DropdownMenu.Trigger>
				{#snippet child({ props })}
					<Button {...props} variant="ghost" size="icon-sm" aria-label="Page options">
						<MoreHorizontal />
					</Button>
				{/snippet}
			</DropdownMenu.Trigger>
		...
		<Button size="sm" class="ml-1 h-7 bg-emerald-500 px-3 text-white hover:bg-emerald-600">
			Publish
		</Button>
	</div>
	...
	<div class="flex items-center gap-1">
		...
		<Tooltip.Root>
			<Tooltip.Trigger>
				{#snippet child({ props })}
					<Button {...props} variant="ghost" size="icon-sm" aria-label="Reset preview">
						<RotateCcw />
					</Button>
				{/snippet}
			</Tooltip.Trigger>
	...
	<div class="flex items-center gap-1">
		...
		<Tooltip.Root>
			<Tooltip.Trigger>
				{#snippet child({ props })}
					<Button {...props} variant="ghost" size="icon-sm" aria-label="Open in new tab">
						<ExternalLink />
					</Button>
				{/snippet}
			</Tooltip.Trigger>
		...
		<Tooltip.Root>
			<Tooltip.Trigger>
				{#snippet child({ props })}
					<Button {...props} variant="ghost" size="icon-sm" aria-label="Fullscreen preview">
						<Maximize2 />
					</Button>
				{/snippet}
			</Tooltip.Trigger>
		...
		<Tooltip.Root>
			<Tooltip.Trigger>
				{#snippet child({ props })}
					<Button {...props} variant="ghost" size="icon-sm" aria-label="Close editor">
						<X />
					</Button>
				{/snippet}
			</Tooltip.Trigger>
