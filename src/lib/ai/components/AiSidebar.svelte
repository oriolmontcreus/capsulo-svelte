<script lang="ts">
	...
	/** bits-ui Viewport is the scrollable child; Root (bind:ref) is overflow-hidden. */
	function scroller() {
		return (
		...
		try {
			...
			if (chat.transcript.at(-1)?.role !== "assistant") {
				chat.transcript.push({
					role: "assistant",
					content: "(This request stopped before I could answer.)",
				});
			}
		} finally {
			endStreamedText();
	...
	async function handleUndo(edit: EditRecord) {
		...
		const entry = chat.entries.find(
			(item) => item.kind === "edit" && item.edit.id === edit.id,
		);
		if (entry?.kind === "edit")
			entry.edit.undoneAt = new Date().toISOString();

		const fields = edit.fields
			.filter((field) => !result.skipped.includes(field))
			.map(
				(field) =>
					`${field.instanceId === "globals" ? "" : `${field.instanceId}.`}${field.fieldName} (${field.locale})`,
			)
			.join(", ");
		if (result.restored > 0) {
			chat.pendingNotes.push(
				`[The user undid your edit to ${edit.targetLabel}: ${fields} are back to their previous values.]`,
			);
		}
...
>
	<Tooltip.Provider delayDuration={150}>
		<header
			class="border-border flex h-11 shrink-0 items-center gap-1 border-b pr-2 pl-4"
		>
			<h2
			...
			<DropdownMenu.Root
				onOpenChange={(open) => open && void refreshChats()}
			>
					...
					{#each chats
						...
						>
							<span
								...
								class:font-medium={item.id === chat.id}
								>{item.title}</span
							>
							<span
		...
		<div class="min-h-0 flex-1">
			<ScrollArea bind:ref={scrollArea} class="h-full w-full">
				<div class="px-6 py-4">
							...
							{#each chat.entries as entry (entry.id)}
								{#if entry.kind === "user"}
									<div class="flex justify-end">
										<div
											class="bg-muted max-w-[85%] rounded-lg px-3 py-2 text-sm whitespace-pre-wrap"
										>
											{entry.text}
										</div>
									</div>
								{:else if entry.kind === "assistant"}
									<div
										class="ai-markdown space-y-2 text-sm leading-relaxed break-words"
									>
...
<style>
	...
	.ai-markdown
		:global(:is(p, ul, ol, pre, blockquote, h1, h2, h3, h4, h5, h6)) {
		margin: 0;
	}
