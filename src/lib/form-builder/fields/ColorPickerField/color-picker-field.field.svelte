      ...
      {:else}
        <div class="color-picker-wrapper">
          ...
          {#if field.presetColors && field.presetColors.length > 0}
            <div
              class="grid grid-cols-5 gap-2 pt-2 border-t border-border mt-2"
            >
...
<style>
  .color-picker-wrapper {
    ...
    --picker-height: 240px;
    --slider-width: 20px;
    /* Unify indicator & thumb to the same size */
    --picker-indicator-size: 17px;
    --thumb-size: 17px;
  }
  ...
  .color-picker-wrapper :global(.picker-indicator),
  .color-picker-wrapper :global(.h .thumb),
  .color-picker-wrapper :global(.a .thumb) {
    cursor: crosshair;
  }

  /* ── Unified indicator & thumb appearance ── */

  /* Picker indicator (the dot on the color area) */
  .color-picker-wrapper :global(.picker-indicator) {
    transition:
  ...
  .color-picker-wrapper :global(.picker:active .picker-indicator) {
    transform: scale(1.4);
    box-shadow: 0 0 6px rgba(0, 0, 0, 0.5);
  }

  /* Slider thumb: override to match picker indicator — white circle with shadow */
  .color-picker-wrapper :global(.h .thumb),
  .color-picker-wrapper :global(.a .thumb) {
  ...
  .color-picker-wrapper :global(.h:active .thumb),
  .color-picker-wrapper :global(.a:active .thumb) {
