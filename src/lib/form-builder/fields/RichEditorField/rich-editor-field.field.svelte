<script lang="ts">
	...
	const toolbarButtonBaseClass =
		"text-foreground/80 hover:bg-muted inline-flex h-7 w-9 items-center justify-center rounded-none font-medium text-sm transition";
	...
	const reactiveEditor = $derived(editor ? makeReactiveEditor(editor) : undefined);
